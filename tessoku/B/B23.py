...
def main():
    ...
    # for i in range(N):
    #     for j in range(N):
    #         print((X[i],Y[i],X[j],Y[j]),distance(X[i],Y[i],X[j],Y[j]))
    ...
    for i in range(1<<N):
        for j in range(N):
            if dp[i][j] >=INF:
                continue
            for k in range(N):
                #ビットが立っていないとき(町kに訪れていないとき)
                #jからkに移動するときのコストを計算して，それをdp[i+(1<<k)][k]に入れる
                if (i>>k)&1==0:
                    test=i+(1<<k)
                    test2=(1<<k)
                    test3=dp[i+(1<<k)][k]
                    test4=dp[i][j]+distance(X[k],Y[k],X[j],Y[j])
                    dp[i+(1<<k)][k]=min(dp[i+(1<<k)][k],dp[i][j]+distance(X[k],Y[k],X[j],Y[j]))
