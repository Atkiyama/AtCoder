...
def main():
    H=W=1501
    coordinates = [[0]*(W+1) for _ in range(H+1)]
    N=int(input())
    for _ in range(N):
    ...
    for j in range(1, W+1):
        for i in range(1, H+1):
            coordinates[i][j] = coordinates[i-1][j] + coordinates[i][j]
            
    for i in range(1, H+1):
        for j in range(1, W+1):
            coordinates[i][j] = coordinates[i][j-1] + coordinates[i][j]
            
    Q=int(input())
    for _ in range(Q):
        a,b,c,d=map(int,input().split())
        ans=coordinates[c][d]-coordinates[a-1][d]-coordinates[c][b-1]+coordinates[a-1][b-1]
        print(ans)
