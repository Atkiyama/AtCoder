...
def main():
    ...
    for _ in range(Q):
        ...
        C+=1
        D+=1
        # print(calcACC(C,D),calcACC(A,D),calcACC(C,B),calcACC(A-2,B-2))
        print(calcACC(C,D)-calcACC(A,D)-calcACC(C,B)+calcACC(A,B))        
...
def calcACC(X,Y):
    ...
    modX=X%N
    modY=Y%N
    return calcACC(N,N)*divX*divY+calcACC(modX,N)*divY+calcACC(N,modY)*divX+calcACC(modX,modY)
