...
def bfs(start):
    ...
    while q:
        ...
        for next in dict[now]:
            ...
            visited[next]=True
            q.append(next)
            set_d(start,next,get_d(start,now)+1)

def get_d(i,x):
    return d_origin[(min(i,x),max(i,x))]

def set_d(i,x,setted):

    d_origin[(min(i,x),max(i,x))]=setted

def in_d(i,x): 
    return (min(i,x),max(i,x)) in d_origin
