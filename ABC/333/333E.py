...
def main():
    ...
    for i in reversed(range(N)):
        if t[i]==2: 
            ...
            K+=1
            kmax=max(kmax,K)
            #history.append("ポーション"+str(k[i])+"をひろう")
        else:
            #history.append("てき"+str(k[i])+"をたおす")
            #history.append("ポーション数"+portiondict)
            if portiondict[k[i]]>0:
                portiondict[k[i]]-=1
                K-=1
                dohisotry.append(1)
            else:
    ...
    for value in portiondict.values():
        if value>0:
            print(-1)
            return
    print(kmax)
    for i in reversed(dohisotry):
        print(i,end=" ")
