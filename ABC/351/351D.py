...
def dfs(x,y):
    visited[x][y]=True
    visited_near_magnet=[[False]*W for _ in range(H)]
    stack=deque([(x,y)])
    count=1
    ...
    while stack:
        ...
        for dx,dy in dxy:
            nx,ny=x+dx,y+dy
            if 0<=nx<H and 0<=ny<W and (not visited[nx][ny] and not visited_near_magnet[nx][ny]):
                if S[nx][ny]==".":
                    stack.append((nx,ny))
                    visited[nx][ny]=True
                    count+=1
                elif S[nx][ny]=="N" and not visited_near_magnet[nx][ny]:
                    count+=1
                    visited_near_magnet[nx][ny]=True
    return count
