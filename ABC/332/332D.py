...
def main():
    # 行と列の順列を生成して全探索
    row_permutations = list(permutations(rows))  # 行の順列をリスト化
    col_permutations = list(permutations(cols))  # 列の順列をリスト化
    ans=INF
    ...
    # col_perm=[0,2,1,4,3]
            ...
            fii=ans
            ans=min(judge(list(row_perm), list(col_perm)),ans)
            # if ans!=fii:
            #     print(row_perm,col_perm)
            #     print(ans)
...
def judge(row_perm, col_perm):
    for i in range(H):
        for j in range(W):
            a=A[i][j]
            b=B[row_perm[i]][col_perm[j]]
            rp=row_perm[i]
            cp=col_perm[j]
    ...
    count=0
    count+=count_min_swaps(row_perm)
    count+=count_min_swaps(col_perm)
